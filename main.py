...
@bot.event
async def on_ready():
  await bot.change_presence(status=discord.Status.online, activity=discord.Game('with Dana\'s feelings'))
...
async def rizz(ctx, user: discord.Member = None):
  ...
  if (user == None):
    ...
    if (rizz > 5):
        await ctx.send("{0} successfully rizzed {2}. They're rizz was {1}/10.".format(ctx.author.mention, rizz, user.mention))
    else:
        await ctx.send("{0} failed to rizz {2}. They're rizz was {1}/10.".format(ctx.author.mention, rizz, user.mention))
